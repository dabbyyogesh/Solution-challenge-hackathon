...
function DirectChat({ auctionId, currentUserEmail }) {
    ...
    useEffect(() => {
        return window.fb.onSnapshot(window.fb.doc(window.db, "auctions", auctionId), (snap) => {
    ...
    const send = async () => {
        ...
        await window.fb.updateDoc(window.fb.doc(window.db, "auctions", auctionId), {
            messages: window.fb.arrayUnion({ sender: currentUserEmail, text: msg.trim(), time: new Date().toLocaleTimeString() })
        });
        setMsg("");
    };
        ...
        <div className="mt-6 pt-6 border-t border-slate-100">
            ...
            <div className="flex gap-2 mt-3">
                <input
                    ...
                    onKeyDown={onKey}
                />
                <button onClick={send} className="btn-primary px-6 py-3">Send</button>
            </div>
        </div>
    );
}
            ...
            if (isLogin) {
                ...
                const profile = {
                    ...
                    ...(role === "employee" && {
                        ...
                        photoURL: formData.photoURL || `https://ui-avatars.com/api/?name=${encodeURIComponent(formData.name || "User")}&background=6366f1&color=fff&rounded=true&bold=true&size=150`,
...
function DirectMessageChat({ currentUser, peer, onClose, setNotifications }) {
    ...
    useEffect(() => {
        ...
        return window.fb.onSnapshot(ref, (snap) => {
            if (snap.exists()) {
                ...
                setChat(msgs);
                // Notify if latest message is from the peer (i.e. not me)
    ...
    const send = async () => {
        if (!msg.trim()) return;
        const ref = window.fb.doc(window.db, "direct_messages", convoId);
        const newMsg = { sender: currentUser.email, text: msg.trim(), time: new Date().toLocaleTimeString() };
        const snap = await window.fb.getDoc(ref);
        if (snap.exists()) {
            await window.fb.updateDoc(ref, { messages: window.fb.arrayUnion(newMsg) });
        } else {
            await window.fb.setDoc(ref, { participants: [currentUser.email, peer.email], messages: [newMsg] });
        }
        setMsg("");
        // Notify self's notification panel that message was sent
        setNotifications(p => [
            { id: Date.now(), text: `✉️ Message sent to ${peer.name || peer.email.split("@")[0]}`, time: new Date().toLocaleTimeString() },
            ...p
        ]);
    };
        ...
        <div className="fixed inset-0 z-[200] flex items-end sm:items-center justify-center p-4 animate-fade-in"
            ...
            <div className="bg-white w-full max-w-md rounded-3xl shadow-2xl border border-slate-100 overflow-hidden animate-slide-up">
                ...
                <div className="flex items-center gap-4 px-6 py-4 border-b border-slate-100 bg-gradient-to-r from-indigo-600 to-purple-600">
                    <img
                        src={peer.photoURL || `https://ui-avatars.com/api/?name=${encodeURIComponent(peer.name || peer.email.split("@")[0])}&background=6366f1&color=fff&rounded=true&bold=true&size=150`}
                ...
                <div className="flex gap-2 p-4 border-t border-slate-100 bg-white">
                    <input
                        ...
                        autoFocus
                    />
                    <button onClick={send} className="btn-primary px-5 py-3 rounded-xl">Send</button>
                </div>
            </div>
        </div>
    );
                            ...
                            <div key={w.id} className="bg-white rounded-3xl overflow-hidden border border-slate-100 card-lift">
                                ...
                                <div className="p-6">
                                    <div className="flex items-start gap-4 -mt-10 mb-4">
                                        <img
                                            src={w.photoURL || `https://ui-avatars.com/api/?name=${encodeURIComponent(w.name || "User")}&background=6366f1&color=fff&rounded=true&bold=true&size=150`}
